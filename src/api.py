"""FastAPI application for ai-shopping-agent."""
...
app = FastAPI(title="ai-shopping-agent", version="0.1.0", description="ai-shopping-agent API")
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "ai-shopping-agent", "version": "0.1.0"}
...
async def process(req: ProcessRequest):
    ...
    try:
        ...
        resp = ProcessResponse(success=True, result=result, processing_time_ms=round(elapsed, 2),
                              metadata={"service": "ai-shopping-agent", "input_length": len(req.input)})
...
@app.get("/status")
def status():
    return {"service": "ai-shopping-agent", "version": "0.1.0", "ready": True,
